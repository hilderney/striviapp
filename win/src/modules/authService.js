...
function createAuthService(options = {}) {
  ...
  return {
    ...
    async changePassword(userId, { currentPassword, newPassword } = {}) {
      ...
      // Só o access token não basta: um token roubado trocaria a senha e
      // tomaria a conta de forma permanente.
      ...
      if (newPassword === currentPassword) {
        throw new AuthError('newPassword must be different from the current password', {
          statusCode: 400,
          code: 'PASSWORD_UNCHANGED',
        });
      }

      const updated = await persistence.updateUser(user.id, {
        passwordHash: hashPassword(newPassword),
      });

      // Revogar antes de emitir o novo par: um refresh token vazado sobreviveria
      // dias à troca de senha, anulando o motivo de trocá-la.
      const revokedSessions = await persistence.revokeRefreshTokensForUser(user.id);
      const session = await buildTokenPair(updated);

      return { changed: true, revokedSessions, ...session };
    },
