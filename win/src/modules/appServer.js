...
async function handleAuthPassword(ctx, req, res, auth) {
  const body = await readJsonBody(req);
  return sendJson(
    res,
    200,
    await ctx.authService.changePassword(auth.userId, {
...
function routeAuthRequest(ctx, req, res, pathname, auth) {
  ...
  if (isPost && pathname === '/api/v1/auth/password') {
    return handleAuthPassword(ctx, req, res, auth);
  }
